...
#define DRIVER_DESC		"MakerBot Industries Desktop 3D Printer"
#define DRIVER_VERSION		"2013 JUL 19"

static DEFINE_MUTEX(printer_mutex);
static const char shortname [] = "3dprinter";
...
MODULE_PARM_DESC(iPNPstring, "MFG:linux;MDL:g_3dprinter;CLS:PRINTER;SN:1;");
...
static struct usb_device_descriptor device_desc = {
	...
	.bDeviceSubClass =	0,
	.bDeviceProtocol =	0,
	.idVendor =		cpu_to_le16(PRINTER3D_VENDOR_NUM),
	.idProduct =		cpu_to_le16(PRINTER3D_PRODUCT_NUM),
...
static void rx_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case 0:
		if (req->actual > 0) {
			...
			DBG(dev, "G_3DPrinter : rx length %d\n", req->actual);
		} else {
...
*/
//static int msg_cleanup(struct fsg_common *common);
...
static int __init printer_bind_config(struct usb_configuration *c)
{
	...
	dev->pdev = device_create(usb_gadget_class, NULL, g_printer_devno,
				  NULL, "g_3dprinter");
	if (IS_ERR(dev->pdev)) {
		ERROR(dev, "Failed to create device: g_3dprinter\n");
		goto fail;
	}
...
init(void)
{
	...
	usb_gadget_class = class_create(THIS_MODULE, "usb_3dprinter_gadget");
...
MODULE_AUTHOR("MakerBot Industries, LLC");
MODULE_LICENSE("GPL");
